...
LLM 도구와 자동화가 여러 머신에서 동시에 돌아가면 “지금 API가 살아 있는지”, “구독 토큰을 얼마나 쓰고 있는지”, “API 과금/토큰 사용이 어디서 발생하는지”를 터미널 로그만으로追적하기 어렵습니다. 이 저장소는 그 상태를 5분 단위로 모아 공개 가능한 JSON과 정적 페이지로 만들고, 자주 보는 정보는 macOS 위젯으로 바탕화면에 올려두는 역할을 합니다.
